...
async function main() {
  console.log('Starting dynamic recipe database sanitization & import...');
  ...
  console.log(`Loaded ${rawRecipes.length} recipes from export file.`);
  
  const cleanRecipes = rawRecipes.filter(r => {
  ...
  console.log(`Filter complete: ${cleanRecipes.length} clean verified recipes remain.`);
  let upsertedCount = 0;
  ...
  for (const r of cleanRecipes) {
    ...
    try {
      ...
      console.error(`  [FAIL] Failed to import recipe: ${titleVal}`, err.message);
    }
  }
  
  console.log(`\nImport complete! Successfully upserted ${upsertedCount} sanitized internal verified recipes.`);
  
  // Safe cascade deletes for UserAction constraints
  ...
  if (badRecipes.length > 0) {
    ...
    console.log(`Cleaned up ${deleteResult.count} unverified or invalid records from database.`);
  } else {
  ...
  console.log(`\nTotal verified internal recipes in local database: ${counts._count.id}`);
}
