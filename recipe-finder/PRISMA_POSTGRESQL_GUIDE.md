# Panduan Migrasi Database PostgreSQL (Supabase / Neon) untuk Racikin

Dokumen ini menjelaskan langkah-langkah detail untuk memigrasikan database platform **Racikin** dari **SQLite** (lokal) ke **PostgreSQL** (production) seperti **Supabase** atau **Neon**.

---

## 1. Perbedaan Setup Lokal vs Production

Platform Racikin dirancang agar fleksibel:
...
2.  Pilih service `recipe-finder` Anda, lalu klik **Edit & Deploy New Revision**.
...
Di platform Racikin yang baru, proses inisialisasi database telah diotomatisasi secara penuh di dalam container startup (`Dockerfile`):
