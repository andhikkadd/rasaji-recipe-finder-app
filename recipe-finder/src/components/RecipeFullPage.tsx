...
export function RecipeFullPage() {
  ...
  useEffect(() => { localStorage.setItem('racikin_likes', JSON.stringify(guestLikes)); }, [guestLikes]);
  useEffect(() => { localStorage.setItem('racikin_bookmarks', JSON.stringify(guestBookmarks)); }, [guestBookmarks]);
  ...
  const handleAiAsk = async (question?: string) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, {
        id: (Date.now() + 1).toString(),
        role: 'assistant',
        content: 'Maaf, terjadi kesalahan saat menghubungi Racikin. Coba lagi nanti ya.'
      }]);
    } finally {
  ...
  // Merge tags and keywords for display
  const allTags = Array.from(new Set([...(recipe.tags || []), ...(recipe.keywords || [])])).slice(0, 5);
    ...
    <div className="recipe-full-page animate-fade-in">
      <div className="page-container">
        ...
        <nav className="breadcrumb-nav">
          <button className="breadcrumb-btn" onClick={() => navigate('/')}>Beranda</button>
        ...
        <header className="hero-header-section">
          ...
          {allTags.length > 0 && (
            <div className="hero-tags">
              {allTags.map((tag, i) => (
                <span key={i} className="hashtag">#{tag.toLowerCase().replace(/\s+/g, '')}</span>
              ))}
            </div>
          )}
                ...
                <div className="support-cards-grid">
                  {recipe.tips && (
                    <div className="support-card support-tips">
                      <h3 className="support-heading">✨ Tips dari Racikin</h3>
                      <p>{recipe.tips}</p>
                    </div>
                  )}
